...
/*   Updated: 2021/04/29 12:30:01 by napark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <cub3d.h>

static void	free_arr(char **s, int i)
{
	while (i--)
		free(s[i]);
	free(s);
...
static int	color_itoa(t_cube3d *s, char *word)
{
	int		len;
	int		n;
...
void		init_bg_color(t_cube3d *s, char *line, int bg_i)
{
	...
	s->bg_color[bg_i - F].bit.r = color_itoa(s, split[0]);
	s->bg_color[bg_i - F].bit.g = color_itoa(s, split[1]);
	s->bg_color[bg_i - F].bit.b = color_itoa(s, split[2]);
	free_arr(split, word_cnt);
}
